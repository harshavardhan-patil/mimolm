...
import src.external.hptr.src.utils.pack_h5 as pack_utils
...
def main():
    ...
    parser.add_argument("--out-dir", default="interim/train")
    ...
    out_path = Path(args.data_dir) / args.out_dir
    out_path.mkdir(exist_ok=True)
    out_h5_path = out_path / (args.dataset + ".h5")

    scenario_list = sorted(list((Path(args.data_dir) / "raw" / args.dataset).glob("*")))
