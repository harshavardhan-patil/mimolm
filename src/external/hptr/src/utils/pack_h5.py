        ...
        if candidate_lanes.any():  # associate to a lane, extend with map topology
            if no_pred:
                ...
                while continue_extend:
                    ...
                    if len(next_edges) == 0:
                        # Fallback to the nearest road edge if lane segment has no successor aka deadend
                        idx_dest = pl_idx_road_edge[np.linalg.norm((pos_road_edge - extended_goal_pos), axis=1).argmin()]
                        break

                    # Choose a random edge and update map IDs
                    dest_map_id, next_map_id = map_edge[np.random.choice(next_edges)]
                    counter += 1

                    # Termination conditions for the loop
                    if (
                ...
                # Final assignment if a valid lane was found
                if len(next_edges) != 0:
