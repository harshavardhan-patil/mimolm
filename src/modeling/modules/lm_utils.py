...
def tokenize_motion(motion_tokens, vocabulary, verlet_wrapper, n_verlet_steps, n_time_steps):
    ...
    motion_tokens = torch.reshape(motion_tokens, (-1, n_time_steps, 2))
    # delta_x and delta_y
    motion_tokens = torch.diff(motion_tokens, dim=1, prepend = motion_tokens[:, :1, :])
    ...
    x_tokens_diff = torch.diff(x_tokens, dim=1, prepend = x_tokens[:, :1])
    y_tokens_diff = torch.diff(y_tokens, dim=1, prepend = y_tokens[:, :1])
...
def get_attention_mask(n_time_steps):
    i = torch.arange(n_time_steps)[:, None]
    j = torch.arange(n_time_steps)
    mask = i >= j
